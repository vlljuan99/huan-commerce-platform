...
class TestCustomerModel:
    ...
    def test_create_b2c_customer(self):
        user = User.objects.create_user(username="retail", email="retail@example.com")
        customer = Customer.objects.create(user=user, segment="b2c")
        assert customer.is_b2c()
        assert not customer.is_b2b()

    def test_create_b2b_customer(self):
        user = User.objects.create_user(
            username="business", email="business@example.com"
        )
        customer = Customer.objects.create(
            user=user, company_name="Acme Corp", tax_id="ES12345678A", segment="b2b"
        )
...
class TestCatalogModels:
    ...
    def test_create_product_category(self):
        category = ProductCategory.objects.create(
            name="Tiles", description="Ceramic tiles"
        )
    ...
    def test_product_variant_stock_reduction(self):
        category = ProductCategory.objects.create(name="Tiles")
        product = Product.objects.create(
            name="Tile", category=category, sku_base="TILE-001"
        )
...
class TestOrderModel:
    ...
    def test_order_status_change(self):
        order = Order.objects.create(order_number="ORD-002", customer=self.customer)
...
class TestCartModel:
    ...
    def setup_method(self):
        ...
        self.product = Product.objects.create(
            name="Tile", category=self.category, sku_base="TILE-001"
        )
    ...
    def test_add_to_cart(self):
        cart = Cart.objects.create(user=self.user)
        item = CartLineItem.objects.create(cart=cart, variant=self.variant, quantity=5)
        assert item.quantity == 5
        assert cart.items.count() == 1

    def test_cart_total(self):
        cart = Cart.objects.create(user=self.user)
        CartLineItem.objects.create(cart=cart, variant=self.variant, quantity=10)
