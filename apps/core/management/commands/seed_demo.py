...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "  Clientes B2B:  construcciones.martin@example.com  /  demo1234\n"
        )
    ...
    def _seed_catalog(self, tax_rates):
        ...
        cat_data = [
            {
                ...
                "slug": "gres-porcelanico",
                "display_order": 1,
            },
            {
                "name": "Azulejos y Revestimientos",
                "slug": "azulejos-revestimientos",
                "display_order": 2,
            },
            {
                "name": "Morteros y Adhesivos",
                "slug": "morteros-adhesivos",
                "display_order": 3,
            },
            {
                "name": "Herramientas y Útiles",
                "slug": "herramientas-utiles",
                "display_order": 4,
            },
        ]
        categories = {}
        for d in cat_data:
            ...
            self.stdout.write(
                f'  Category [{("creada" if new else "ya existía")}]: {obj}'
            )
        ...
        brand_data = [
            {
                "name": "Porcelangres",
                "slug": "porcelangres",
                "description": "Fabricante español de gres porcelánico técnico",
            },
            {
                "name": "Butech",
                "slug": "butech",
                "description": "Sistemas de colocación cerámica de Porcelanosa",
            },
            {
                "name": "Ardex",
                "slug": "ardex",
                "description": "Morteros y adhesivos de alta resistencia",
            },
            {
                "name": "Rubi",
                "slug": "rubi",
                ...
                "variants": [
                    {
                        "sku": "GP-MQ-60-MATE",
                        "name": "Acabado Mate",
                        "price_no_tax": Decimal("22.50"),
                        "stock_quantity": 240,
                    },
                    {
                        "sku": "GP-MQ-60-PORC",
                        "name": "Acabado Pulido",
                        "price_no_tax": Decimal("25.80"),
                        "stock_quantity": 180,
                    },
                ],
            },
        ...
        for spec in products_spec:
            ...
            self.stdout.write(
                f'  Product [{("creado" if new else "ya existía")}]: {prod}'
            )
            ...
            for v_data in spec["variants"]:
                ...
                self.stdout.write(
                    f'    Variant [{("creada" if new_v else "ya existía")}]: {v.sku}'
                )
    ...
    def _seed_users_and_customers(self):
        ...
        for spec in users_spec:
            ...
            self.stdout.write(
                f'  User [{("creado" if new else "ya existía")}]: {user.email}'
            )
            ...
            if spec.get("customer"):
                ...
                self.stdout.write(
                    f'    Customer [{("creado" if new_c else "ya existía")}]: {cust}'
                )
                ...
                for addr_data in spec.get("addresses", []):
                    ...
                    self.stdout.write(
                        f'      Address [{("creada" if new_a else "ya existía")}]: {addr.name}'
                    )
    ...
    def _seed_orders(self, customers, variants):
        ...
        def _dt(year, month, day):
            return timezone.make_aware(datetime.datetime(year, month, day, 9, 0, 0), tz)
        ...
        orders_spec = [
            # ── Pedido 1: Martín, entregado ───────────────────────────
            {
                ...
                "lines": [
                    {
                        "variant_sku": "GP-MQ-60-MATE",
                        "qty": Decimal("85.00"),
                        "price": Decimal("22.50"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "AD-C2TE-25-GR",
                        "qty": Decimal("40.00"),
                        "price": Decimal("12.80"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "LE-FL-5-GR",
                        "qty": Decimal("20.00"),
                        "price": Decimal("6.90"),
                        "tax_pct": Decimal("21.00"),
                    },
            ...
            # ── Pedido 2: Martín, procesando ──────────────────────────
            {
                ...
                "lines": [
                    {
                        "variant_sku": "GP-CG-45-GR",
                        "qty": Decimal("120.00"),
                        "price": Decimal("14.90"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "GP-CG-45-AN",
                        "qty": Decimal("60.00"),
                        "price": Decimal("14.90"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "AD-C2TE-25-WH",
                        "qty": Decimal("60.00"),
                        "price": Decimal("12.80"),
                        "tax_pct": Decimal("21.00"),
                    },
            ...
            # ── Pedido 3: Rodríguez, confirmado ───────────────────────
            {
                ...
                "lines": [
                    {
                        "variant_sku": "AZ-HID-20-AZ",
                        "qty": Decimal("32.00"),
                        "price": Decimal("18.40"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "LE-FL-5-BL",
                        "qty": Decimal("8.00"),
                        "price": Decimal("6.90"),
                        "tax_pct": Decimal("21.00"),
                    },
            ...
            # ── Pedido 4: Retail B2C, pendiente ───────────────────────
            {
                ...
                "lines": [
                    {
                        "variant_sku": "AZ-HID-20-VE",
                        "qty": Decimal("12.00"),
                        "price": Decimal("18.40"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "LE-FL-5-ANT",
                        "qty": Decimal("3.00"),
                        "price": Decimal("7.20"),
                        "tax_pct": Decimal("21.00"),
                    },
                    {
                        "variant_sku": "HE-TX1020-STD",
                        "qty": Decimal("1.00"),
        ...
        for spec in orders_spec:
            ...
            for line in spec["lines"]:
                ...
                line_tax = (line_sub * line["tax_pct"] / Decimal("100")).quantize(
                    Decimal("0.01")
                )
            ...
            self.stdout.write(
                f"  Order [creado]: {order.order_number}  total={order.total} €"
            )
